...
Публичный webhook развёрнут как отдельный API-only Site: `https://estate-radar-ingress.iolzhik220366.chatgpt.site/api/integrations/krisha/moderation/events`. Там доступны только health-check, HMAC-подписанный приём событий и admin-защищённое управление подписками. Основной Estate Radar CRM сохраняет прежний закрытый режим.

На webhook Site настроены `KRISHA_MODERATION_WEBHOOK_SECRET` и отдельный `KRISHA_MODERATION_ADMIN_KEY`; локальная копия для подключения исходного сервиса находится в игнорируемом `.env.ingress.local` соседнего проекта `estate-radar-ingress`. Настройте `TELEGRAM_BOT_TOKEN` и минимум одну подписку на admin-маршруте до включения исходящего отправителя: пока их нет, webhook сохраняет подписанное событие и отвечает `503`, чтобы Krisha повторила доставку после настройки. Текущие переменные среды не содержат Telegram token или chat ID, а исходный сервис Krisha в доступном workspace отсутствует, поэтому production Telegram-доставка и интеграция upstream пока не завершены.

Публичная проверка `GET /api/health` возвращает `200`; production D1 содержит таблицы событий, outbox и подписок. Подписанный синтетический запрос дошёл до проверки контракта и получил `400 invalid_moderation_event`; данные объявления не записывались. Успешное `202` и Telegram-сообщение необходимо подтвердить после настройки токена/подписки и подключения отправителя из сервиса, который фиксирует переход объявления на модерацию.
